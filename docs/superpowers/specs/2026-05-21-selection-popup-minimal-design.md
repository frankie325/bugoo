...
3. macOS 未授权时，打开一个应用内权限提示窗口，提供“去系统设置”和“稍后”操作；本次运行只提示一次，并且暂不启动划词监听。
4. 权限提示窗口保持打开时，后端定时轮询 Accessibility 权限；一旦授权成功，关闭权限提示窗口并启动划词监听。
5. macOS 已授权或非 macOS 平台时，使用 `rdev` 注册全局鼠标事件监听。
6. 监听到鼠标左键释放后，调用 `get_selected_text` 工具包读取当前系统选中文本。
7. 如果读取结果 trim 后为空，则直接结束，不打开也不更新弹窗。
8. 对非空候选文本做过滤：长度超过 50 个字符不弹。
9. 调用窗口命令打开或更新 selection popup 浮窗。
...
- 权限提示窗口打开期间，定时轮询 Accessibility 权限状态。
- 如果轮询发现权限已授权，关闭权限提示窗口并启动 `rdev` 监听。
- 如果用户点击“稍后”，关闭权限提示窗口并停止本次轮询；本次运行不再重复提示。
...
- 权限提示窗口打开期间，后端每隔约 1-2 秒检查一次 Accessibility 权限。
- 如果用户在系统设置中完成授权，应用自动关闭权限提示窗口并启动划词监听，不要求重启应用。
...
- 如果未授权，打开应用内权限提示窗口。
- 权限提示窗口提供跳转系统设置的入口。
- 权限提示窗口保持打开时，轮询检测 Accessibility 权限变化。
- 轮询检测到授权成功后，关闭权限提示窗口并启动 `rdev` 划词监听。
- 用户点击“稍后”后，本次运行不再重复提示，也停止权限轮询。
...
App setup
  ...
  -> while permission window is open: poll Accessibility permission
  -> permission granted: close permission window and start rdev listener
...
- 权限轮询失败：记录 debug 或 warn，继续下一次轮询。
- 权限轮询检测到授权成功：关闭权限提示窗口并启动划词监听。
- 用户点击“稍后”：关闭权限提示窗口，停止本次权限轮询。
...
- macOS 权限提示窗口轮询到授权成功后启动 selection listener。
- 用户点击“稍后”后停止权限轮询，且本次运行不重复提示。
...
- 未授权时不启动划词弹窗链路，不弹空窗。
- 权限提示窗口保持打开时，在系统设置中授权后无需重启即可启动划词监听。
