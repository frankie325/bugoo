...
本文件为 Codex（Codex.ai/code）在处理本代码库时提供指导。
...
补充说明：`src-tauri/tauri.conf.json` 已配置 `beforeDevCommand: pnpm dev` 和 `beforeBuildCommand: pnpm build`，启动/构建 Tauri 时会自动衔接前端流程。
...
| `--color-warning-soft` / `-foreground` / `-hover` | 警告软底 | 同上 `bg-warning-soft` |
| `--color-danger` / `-hover` / `-foreground` | 错误状态 | `bg-danger` / `text-danger` |
| `--color-danger-soft` / `-foreground` / `-hover` | 错误软底 | 同上 `bg-danger-soft` |
| `--color-backdrop` | 模态背景遮罩 | `bg-backdrop` |

### ⚠️ HeroUI v3 中**不存在**的样式

下列样式名**不存在**于 HeroUI v3，使用会得到**无效果**或被 Tailwind 当成普通类名忽略：

- ~~`text-default-400/500/600/700/900`~~（错误：缺少数字阶 token）
- ~~`bg-default-50/100/200`~~（错误：缺少数字阶 token）
- ~~`text-primary` / `bg-primary` / `border-primary`~~
- ~~`text-accent-1/2/.../10`~~（项目自定义了 `accent-1`~`accent-10` 数字阶，注意**与 HeroUI 的 `--color-accent` 语义不同**）

### 项目自定义变量

`src/styles/variable.css` 通过 `@theme { --color-accent-N: ... }` 导出的**品牌绿色阶**（仅品牌主色相关）：

| CSS 变量 | 来源 | 用途 |
|---------|------|------|
| `--color-accent-1` ~ `--color-accent-10` | 项目 variable.css | 品牌绿色阶（light/dark 各 10 阶） |
| `--accent` (= `--accent-6` light / `--accent-6` dark) | 项目 variable.css | 品牌主色 alias |

**注意：项目自定义的 `accent-1`~`accent-10` 与 HeroUI 的 `--color-accent`（单一品牌色）不同名空间。** 当需要品牌色阶（如图标背景 + 文字的颜色对比）时使用 `accent-N`；当需要单一品牌色（如按钮、强调文字）时用 HeroUI 的 `text-accent` / `bg-accent`。

### 如何添加新颜色变量

如果 HeroUI v3 和项目 variable.css 都不满足需求，按以下规则扩展：
...
2. **如果确实需要新的色阶**，在 `src/styles/variable.css` 中按 Tailwind v4 `@theme { --color-xxx-N: ... }` 格式定义：
   ...
   ```
3. 然后在 `@theme inline` 中将 HeroUI 的 `--color-foo` 变量 alias 到你的新色阶（如果需要覆盖）
4. 通过 `bg-my-purpose-50` / `text-my-purpose-100` 等 Tailwind 工具类使用

### 验证清单

- [ ] 不用 `text-[#xxx]` / `bg-[#xxx]` / `border-[#xxx]`
- [ ] 不使用 HeroUI v3 中**不存在**的 token（见上方清单）
